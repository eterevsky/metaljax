...
| 2 | gemma4-12B bf16 | 314.1 (f32) | **56.2** ¹² (26 GB) | 58.3 ⁷ | 67.6 | 44.2 ¹⁰ |
| 3 | gemma4-26B-A4B (MoE) | ✗ guard-killed @34 GB ⁶ | **31.1** ¹² | **17.0** | — | 16.9 ¹⁰ |
| 4 | gemma4-E2B bf16 | 67.4 (bf16→f32) ⁵ | **16.9** ¹² | 10.5 ⁷ | — | — |
| 5 | Qwen3-8B bf16 | 205.0 (bf16→f32) ⁵ | **36.0** (17 GB) | 30.4 | 38.1 | 29.6 ¹⁰ |
| 6 | Llama-3.1-8B bf16 | 203.9 (bf16→f32) ⁵ | **37.3** | 29.4 | 35.5 | 29.2 ¹⁰ |
| 7 | gpt-oss-20b | ✗ ¹ | **13.2** ¹² (34 GB) | **8.8** (13.8 GB, native MXFP4) | — | 6.7 ¹⁶ |
| 8 | Qwen3.6-35B-A3B (MoE) | ✗ 144 GB | **23.3** (73 GB) | **13.7** | — | 15.3 ¹⁰ |
| 9 | R1-Distill-32B | ✗ 131 GB | **199.7** ¹² (69 GB) | 131.8 | — | 114.9 ¹⁰ |
| 10 | DeepSeek-V2-Lite (maxtext) | ✗ needs 50–105 GB ² | **19.8** ¹¹ (90 GB) | 10.5 | — | 10.7 ¹⁰ |
| 11 | Qwen3-0.6B (keras-hub decode) | 28.5 | **5.1** ¹³ | 3.2 ¹³ | — | 3.4 ¹⁰ |
| 12 | Mixtral 8×7B bf16 | ✗ | **70.8** (93 GB) | 53.5 ¹⁷ (93.4 GB) | — | — |
| 13 | gemma4-E2B keras-int4 (packed) | 67.4 ⁸ | **6.0** ⁸ (48 GB load peak) | 4.5 ¹⁹ | — | — |
| 14 | Qwen3-0.6B maxtext qwix-int8 | 144.4 | **26.37** | — | — | — |
| 15 | *qwix-int8 Qwen3-8B* | 2118 | **267.7** (52 GB) | — | — | — |
| 16 | SigLIP 2 (fwd b1 ms) | 358.5 | **41.86** | — | 29.8 (b32: 591) | — |
| 17 | SD 3.5 Large (ms/diff-step) | ✗ ⁴ | **462.5** @512², **2061.8** @1024² ¹² | ✗ ⁹ | 553 @512², 3078 @1024² ⁹ ¹⁸ | — |
| 18 | LoRA gemma4-E2B train (ms/step) | 1360 | **119.8** ¹² | — | 135.6 ³ | — |
| 19 | Qwen3-0.6B maxtext train (ms/step) | 1406 | **362.8** | — | 818 ²⁰ | — |
| 20 | *aspirational* Qwen3-235B-A22B 3-bit (mlx quant) | ✗ | **45.2** (103 GB) | **28.0** (102.9 GB, load 12 s) | — | — |
| 21 | Qwen3.8-27B bf16 (dense hybrid) | ✗ ¹⁴ | **142.5** (57 GB) | **106.4** ¹⁵ | — | 98.2 ¹⁰ |
...
8. Row 13: every cell through 0.11.7 (metaljax 77.0, jax-CPU 67.8 and
   ...
   CPU cell is that variant at the row protocol on jax 0.11.2 (67.4, 128
   tokens; its stream now equals metaljax's 64/64); the
...
11. Row 10 protocol: runs with `METALJAX_MEM_SYS_MB=107520` (its
    ...
    steps, on the 0.11.8 release binary (19.76, jax 0.11.0 maxtext venv) — the
    comparators' workload; the harness variant `MAXTEXT_RNG_PRESPLIT=1`
    (the RNG split hoisted out of the timed loop) reads 19.13 on the same
...
12. The 0.11.8 cells: gate of 2026-09-23 on the release binary, jax 0.11.2
    ...
    0.11.2); the jax-CPU cells are jax 0.11.2's (it moved LoRA 2316 -> 1360 and
    SigLIP 537 -> 359).  Greedy agreement vs jax-CPU: rows 2/5/6/13 exact
    64/64; row 4 parts at 51/64 (the certified-benign tie).  Every metaljax
    stream equals the 2026-09-21 gate's; against 0.11.7 rows 4/5/6/7/20/21
    are identical, row 2 flipped back to the CPU stream, rows 1 and 3 carry
    tie flips (notes/release-gates-0.11.8.md).  In-battery vs standalone (the
    suite-context class): row 3 31.1 vs 28.2 / 28.3; row 18 119.8 vs
    113.0-113.8 on either binary and either jax (the in-battery cells are the
    table's, as in 0.11.7); row 17 at 1024² 2081.0 in sequence vs 2061.8
    standalone (the standalone cell is the table's); row 9 185-200 across six
    draws on two binaries (a ±4 % row).
