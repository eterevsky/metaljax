...
| `METALJAX_MATMUL_PRECISION` | `high` | On M5-class GPUs MLX can route GEMM, attention and quantized matmul through the neural accelerators, which for f32 means ~bf16 input precision (~8e-4 error vs f64). `high` keeps f32 on the exact kernels (~1e-6) while bf16/f16 and quantized matmuls use the accelerators (MLX's own `MLX_ENABLE_TF32=0`); `highest` pins every kernel to the previous GPU generation (no accelerator kernels for any dtype -- 2x slower on bf16 forward/training rows); `default` is MLX's own default (accelerator f32 too). |
