...
| `METALJAX_STACKED_RELAYOUT` | `1` | debug-bisect | plugin-native only: =0 declines the stacked dot's RELAYOUT form (a stack whose contracted axes straddle the layer axis -- maxtext's attention out-projection -- is packed once per executable in its carried layout, B3); off, the slice chain runs as before (the sibling packs, `METALJAX_STACKED_PACK`, are a separate switch) |
...
| `METALJAX_STACKED_PACK` | `auto` | user knob | plugin-native only: the stacked SIBLING pack (`metal_stacked.cc`, row 10 rewrite 3): several stacked dots in one block that read the SAME activation against the same layer index and DIFFERENT per-layer stacks (row 10's q + kv_a over the attention norm; its router + the two shared-expert gates over the MLP norm) become ONE `gather_mm` over the stacks' [L, K, N] views concatenated on the free axis, materialized once per executable out of the RELAYOUT's budget (`METALJAX_STACKED_RELAYOUT_MB`, governor-admitted) with per-member last-axis slice VIEWS. `0` declines every group (the members keep their own dots); unset/`1`/`auto` = `metal_proj.cc`'s policy verbatim (in a group of >= 3 with one member at least twice as wide as the next, the wide one is left alone when the rest, packed, stay in their own gemv tile); `all` packs every member of every group. Decode-only (M == 1, the relayout's rule); a RELAID member never joins a group (its own pack is built first and packing it again would hold the stack twice), a per-layer matrix over 64 MiB is not a member (bandwidth-bound already), and the same stack twice in one group is refused. Bit-identical to the member dots: the pack is their own [L, K, N] layout with a larger N, and a `gemv_t`'s per-output K order does not depend on N below K = 8192 (verified bit-exact by `execute_test.py`'s `_p46_stacked_pack` and by stream identity on row 10). Every decision narrated under METALJAX_DEBUG=1 (`stacked:` lines) |
...
| `METALJAX_LOOP_SPECIALIZE` | `1` | user knob | plugin-native only: =0 replays the single generic body; on (default) compiles a counted loop's body ONCE PER CHUNK with the loop position folded into its dynamic slice/update starts (P49). The per-layer stack reads become `shared_buffer_slice` views (no `compute_dynamic_offset` kernel, no row copy) and the stacked-accumulator writes lose their offset kernel while keeping the same donation test and window copy -- bit-identical, and measured -49 % dispatches on the `_p49` synthetic. Costs one `mx::compile` trace per chunk on the first execute. On by default since 2026-09-21 (row 10 -1.2 ms/tok, rows 11/14 flat, texmo gate 106/106 in this mode). |
| `METALJAX_LOOP_SPECIALIZE_MAX` | `32` | user knob | plugin-native only: most position-specialised body variants one loop may build (`trip/K + trip%K`). Past it the loop declines and replays the single generic body; 0 disables the rewrite for every loop. |
