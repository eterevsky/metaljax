...
| **15** qwix-int8 Qwen3-8B (maxtext) | **COMPLETE**, exit 0 — decode 369.7 ms/tok, load 80.5 s; output text is wrong (`" fragment!!!!!!!"` = token ids `[12289,0,0,0,0,0,0,0]`; `!` is Qwen3's token 0, i.e. **collapsed logits**). Not memory — but the attribution written here first ("the row's known MLX-quantization bug") is **WITHDRAWN**: no such finding exists (2026-08-03 exonerated the quantized dots, `7932b4d`), the mechanism is OPEN, and the timing is not publishable as a cell. STATUS footnote 34, `notes/row15-wrong-output-2026-08-17.md` | 79.0 GB | flat | 13 lines, 0 refusals | predicted "completes or clean OOM" — **completes** |
| **15f** `MAXTEXT_PREFILL_LEN=64 METALJAX_BODY_COMPILE=0` | **COMPLETE**, exit 0 — decode 1064 ms/tok (the mitigation's cost); output *worse*, `"!!!!!!!!"` — all 8 tokens are id 0, the collapse now reaches the first token. NB the `MAXTEXT_PREFILL_LEN=64` half is a **no-op** (the adapter already rounds a 5-token prompt up to 64), exactly as in row 10f — so this IS a clean single-variable arm on `METALJAX_BODY_COMPILE=0`, and the collapse survives turning body compilation off | 75.0 GB | flat | 7 lines, 0 refusals | the fix lowers the peak 4 GB and costs 2.9× |
