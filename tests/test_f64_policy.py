...
def test_f64_passthrough_allowed():
    ...
    (out,) = run_module(lower_x64(f, np.float64(1.5)), (np.float64(1.5),))
...
def test_f64_arithmetic_rejected_at_compile():
    ...
    with pytest.raises(UnsupportedDtypeError, match="float64"):
        engine.compile_program(data, "mlir")
...
def test_c128_passthrough_allowed():
    ...
    (out,) = run_module(lower_x64(f, np.complex128(1 + 2j)),
                        (np.complex128(1 + 2j),))
    np.testing.assert_array_equal(out, np.complex64(2 + 4j))
